import { useState } from "react";
import { HiHome } from "react-icons/hi2";
import { IoSettings, IoLogOut } from "react-icons/io5";
import { RiMessage3Fill } from "react-icons/ri";
import { FaRegMoneyBillAlt } from "react-icons/fa";
import { BiSolidVideos } from "react-icons/bi";
import { useDispatch } from "react-redux";
import { setUserEmail } from "../../../redux/auth";
import authService from "../../../services/authService";
import { toast } from "react-toastify";

const SideNav = ({ isOpen, onClick, hasSelected, type }) => {
  const [selected, setSelected] = useState(0);
  const dispatch = useDispatch();

  const handleClick = (index, component) => {
    setSelected(index);
    onClick(component);

    // hasSelected();
    // dispatch(setSidenavOpen(false));
  };

  const handleLogout = async () => {
    await authService.logout();
    toast.info("Logged Out Successfully");
    dispatch(setUserEmail(null));
    window.location.href = "/";
  };

  return (
    <nav className="flex flex-col ">
      <div
        className={`${
          isOpen
            ? "bg-neutral-400 bg-opacity-95 "
            : "invisible w-0 h-0 lg:w-auto lg:h-auto lg:visible -translate-x-full md:-translate-x-0"
        } md:flex md:flex-shrink-0 transition duration-500 ease-in-out transform `}
      >
        <div className="px-2 py-6 h-screen w-[14rem] border-r-[0.08rem] bg-secondary border-gray-200 text-gray-700 flex flex-col justify-between">
          <div className="flex flex-col gap-5">
            <div className=" p-2"></div>
            <div
              onClick={() => handleClick(3, "Feeds")}
              className={`${
                selected === 3
                  ? "border-[#0f7173] border-l-2 text-white"
                  : "hover:border-gray-300 "
              } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
            >
              <div
                className={`${
                  selected === 3
                    ? "  bg-[#0f7173]"
                    : " hover:bg-gray-300 hover:text-gray-800"
                } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
              >
                <RiMessage3Fill />
                <div className="cursor-pointer">Feeds</div>
              </div>
            </div>
            <div
              onClick={() => handleClick(0, "Home")}
              className={`${
                selected === 0
                  ? "border-[#0f7173] border-l-2 text-white"
                  : "hover:border-gray-300 "
              } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
            >
              <div
                className={`${
                  selected === 0
                    ? "  bg-[#0f7173] "
                    : " hover:bg-gray-300 hover:text-gray-800"
                } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
              >
                <HiHome />
                <div className="cursor-pointer">Home</div>
              </div>
            </div>

            <div
              onClick={() => handleClick(2, "Videos")}
              className={`${
                selected === 2
                  ? "border-[#0f7173] border-l-2 text-white"
                  : "hover:border-gray-300 "
              } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
            >
              <div
                className={`${
                  selected === 2
                    ? "  bg-[#0f7173]"
                    : " hover:bg-gray-300 hover:text-gray-800"
                } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
              >
                <BiSolidVideos />
                <div className="cursor-pointer">Videos</div>
              </div>
            </div>

            {type === "brand" && (
              <div
                onClick={() => handleClick(5, "Subscriptions")}
                className={`${
                  selected === 5
                    ? "border-[#0f7173] border-l-2 text-white"
                    : "hover:border-gray-300 "
                } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
              >
                <div
                  className={`${
                    selected === 5
                      ? "  bg-[#0f7173]"
                      : " hover:bg-gray-300 hover:text-gray-800"
                  } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
                >
                  <FaRegMoneyBillAlt />
                  <div className="cursor-pointer">Subscriptions</div>
                </div>
              </div>
            )}

            <div
              onClick={() => handleClick(6, "Settings")}
              className={`${
                selected === 6
                  ? "border-[#0f7173] border-l-2 text-white"
                  : "hover:border-gray-300 "
              } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
            >
              <div
                className={`${
                  selected === 6
                    ? "  bg-[#0f7173]"
                    : " hover:bg-gray-300 hover:text-gray-800"
                } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
              >
                <IoSettings />
                <div className="cursor-pointer">Settings</div>
              </div>
            </div>

            <div
              onClick={() => handleLogout()}
              className={`${
                selected === 7
                  ? "border-[#0f7173] border-l-2 text-white"
                  : "hover:border-gray-300 "
              } flex items-center gap-2  px-2 hover:border-l-2 transition ease-in-out delay-50`}
            >
              <div
                className={`${
                  selected === 7
                    ? "  bg-[#0f7173]"
                    : " hover:bg-gray-300 hover:text-gray-800"
                } flex items-center px-2 gap-2 rounded-lg w-[10rem] transition ease-in-out delay-50`}
              >
                <IoLogOut />
                <div className="cursor-pointer">Log Out</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </nav>
  );
};

export default SideNav;
